...
def process_pdf_file_for_tables(pdf_path):
    # Find the PDF path
    #pdf_path = '/content/OMM_Rubric.pdf'
    ...
    for pagenum, page in enumerate(extract_pages(pdf_path)):
        st.write(f"Processing page {pagenum}")
...
if st.button("Extract Tables"):
    text_per_page = process_pdf_file_for_tables("uploaded_file.pdf")
    print("Starting write page...")
    full_rubric = ""
    for k,v in text_per_page.items():
        print("Writing another page...")
        content = ''.join(v[4])
        full_rubric=full_rubric+content
        st.write(content)
        print(content)
    print("Finished write page")
    with open("rubric.txt", "w") as f:
        f.write(full_rubric)
        
    #total="".join([text_per_page[t][4] for t in text_per_page])
    #st.write(f"# Total text: \n{total}")